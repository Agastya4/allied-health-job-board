-- Production database setup with proper error handling
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jobs_location ON jobs USING GIN(to_tsvector('english', location_display));
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jobs_title ON jobs USING GIN(to_tsvector('english', job_title));
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jobs_categories ON jobs USING GIN(job_categories);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_job_bookmarks_user_id ON job_bookmarks(user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_job_bookmarks_job_id ON job_bookmarks(job_id);
...
('employer@alliedhealthjobs.com.au', 'Demo Employer', '$2a$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj/VcSAg/9qm', 'employer'),
('jobseeker@alliedhealthjobs.com.au', 'Demo Job Seeker', '$2a$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj/VcSAg/9qm', 'job_seeker')
ON CONFLICT (email) DO NOTHING;

-- Insert comprehensive sample jobs
INSERT INTO jobs (
...
• Work with children with autism, ADHD, developmental delays, and sensory processing disorders
• Collaborate with families, educators, and other healthcare professionals
...
(1, 'recruitment@speechpathways.com.au', 'Speech Pathologist', 'Brisbane QLD 4000 Australia', 'Brisbane, QLD',
 ...
 'Speech Pathways is seeking a dedicated Speech Pathologist to join our community health team in Brisbane. Perfect opportunity for work-life balance.
...
• Develop and implement evidence-based intervention programs
• Provide education and training to families and carers
...
• Access to latest assessment tools and technology', 
...
(1, 'jobs@mindwellness.com.au', 'Clinical Psychologist', 'Perth WA 6000 Australia', 'Perth, WA',
 ...
 'MindWellness Psychology is expanding our team with an experienced Clinical Psychologist. Join our innovative mental health practice.

Key Responsibilities:
• Provide psychological assessment and intervention services
• Work with adults experiencing anxiety, depression, trauma, and adjustment disorders
• Conduct individual and group therapy sessions
• Develop comprehensive treatment plans
• Collaborate with psychiatrists and other mental health professionals
...
• Modern facilities in Perth CBD', 
...
(1, 'careers@nutritionplus.com.au', 'Dietitian', 'Adelaide SA 5000 Australia', 'Adelaide, SA',
 ...
 'NutritionPlus is seeking a qualified Dietitian for a 12-month contract position at Adelaide''s premier private hospital.
...
• Collaborate with medical and nursing staff
